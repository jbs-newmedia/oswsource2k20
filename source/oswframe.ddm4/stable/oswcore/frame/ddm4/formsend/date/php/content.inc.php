...
if ($this->getSendElementOption($element, 'year_sortorder') === 'desc') {
    for ($i = $this->getSendElementOption($element, 'year_max'); $i >= $this->getSendElementOption(
        $element,
        'year_min'
    ); $i--) {
    ...
    for ($i = $this->getSendElementOption($element, 'year_min'); $i <= $this->getSendElementOption(
        $element,
        'year_max'
    ); $i++) {
        $date['year'][sprintf('%02d', $i)] = sprintf('%02d', $i);
