    ...
    <?php if ($this->getSendElementOption($element, 'read_only') === true): ?>
        ...
        <div
            class="form-control readonly"><?php echo HTML::outputString(
                $this->getSendElementOption($element, 'text_hidden')
            ) ?></div>

    <?php else: ?>

        <?php /* input */ ?><?php echo $this->getTemplate()->Form()->drawPasswordField(
            $element,
...
<div class="form-group ddm_element_<?php echo $this->getSendElementValue($element . '_double', 'id_double') ?>">
    ...
    <label class="form-label"
           for="<?php echo $element . '_double' ?>"><?php echo HTML::outputString(
               $this->getSendElementOption($element, 'title_double')
           ) ?><?php if ($this->getSendElementOption($element, 'required') === true): ?><?php echo $this->getGroupMessage(
               'form_title_required_icon'
           ) ?><?php endif ?><?php echo $this->getGroupMessage('form_title_closer') ?></label>
    ...
    <?php if ($this->getSendElementOption($element . '_double', 'notice') !== ''): ?>
        <div
            class="text-info"><?php echo HTML::outputString(
                $this->getSendElementOption($element . '_double', 'notice')
            ) ?></div>
    <?php endif ?>
