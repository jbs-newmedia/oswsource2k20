...
foreach ($this->getAddElements() as $element => $options) {
    if (!in_array($element, $ajax['init'], true)) {
...
foreach ($ajax['logic'] as $group => $group_data) {
    foreach ($group_data['rule'] as $rule_key => $rule_value) {
        $this->parseElementPHP('datatableajax', $this->getAddElementValue($rule_key, 'module'), [
            'module' => $this->getAddElementValue($rule_key, 'module'),
            'rule_key' => $rule_key,
    ...
    foreach ($ajax['logic'][$group]['view'] as $element) {
        ...
        if (isset($ajax['logic'][$group]['reset'])) {
            foreach ($ajax['logic'][$group]['reset'] as $reset_key) {
                $this->parseElementPHP('datatableajax', $this->getAddElementValue($reset_key, 'module'), [
                    'module' => $this->getAddElementValue($reset_key, 'module'),
                    'reset_key' => $reset_key,
                ]);
        ...
        if (isset($ajax['logic'][$group]['set'])) {
            foreach ($ajax['logic'][$group]['set'] as $set_key => $set_value) {
                $this->parseElementPHP('datatableajax', $this->getAddElementValue($set_key, 'module'), [
                    'module' => $this->getAddElementValue($set_key, 'module'),
                    'set_key' => $set_key,
                    'set_value' => $set_value,
                ]);
