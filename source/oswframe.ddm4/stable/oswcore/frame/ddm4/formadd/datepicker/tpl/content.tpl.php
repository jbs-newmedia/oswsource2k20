...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($element, 'id') ?>">
    ...
    <?php if ($this->getAddElementOption($element, 'read_only') === true): ?>
        ...
        <div class="form-control readonly">
            ...
            <?php if (($this->getAddElementStorage($element) === '') || ($this->getAddElementStorage(
                $element
            ) === '0') || ($this->getAddElementStorage(
                $element
            ) === '00000000') || ($this->getAddElementStorage($element) === '0')
            ): ?>
                ---
            <?php else: ?>
                ...
                <?php echo $this->getTemplate()->Form()->drawHiddenField(
                    ...
                    \osWFrame\Core\DateTime::strftime(
                        $this->getAddElementOption($element, 'format'),
                        mktime(
                            12,
                            0,
                            0,
                            substr($this->getAddElementStorage($element), 4, 2),
                            substr($this->getAddElementStorage($element), 6, 2),
                            substr($this->getAddElementStorage($element), 0, 4)
                        )
                    )
                ) ?><?php if ($this->getAddElementOption(
                    $element,
                    'month_asname'
                ) === true
                ): ?><?php echo \osWFrame\Core\DateTime::strftime(
                    str_replace('%m.', ' %B ', $this->getAddElementOption($element, 'format')),
                    mktime(
                        12,
                        0,
                        0,
    ...
    <?php else: ?>
        ...
        <?php if (($this->getAddElementStorage($element) === '') || ($this->getAddElementStorage(
            $element
        ) === '0') || ($this->getAddElementStorage($element) === '00000000') || ($this->getAddElementStorage(
            $element
        ) === '0')
        ): ?>
        ...
        <?php else: ?>
            ...
            <?php echo $this->getTemplate()->Form()->drawTextField(
                $element,
                \osWFrame\Core\DateTime::strftime(
                    $this->getAddElementOption($element, 'format'),
                    mktime(
