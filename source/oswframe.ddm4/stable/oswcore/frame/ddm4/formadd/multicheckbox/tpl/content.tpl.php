...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($element, 'id') ?>">
    ...
    <?php if ($this->getAddElementOption($element, 'read_only') === true): ?>
        ...
        <?php /* read only */ ?><?php $multicheckbox = [] ?><?php if ($this->getAddElementStorage(
            $element
        ) !== ''
        ): ?><?php $multicheckbox = explode(
            $this->getAddElementOption($element, 'separator'),
            $this->getAddElementStorage($element)
        ) ?><?php endif ?><?php if ($this->getAddElementOption(
            $element,
            'orientation'
        ) === 'horizontal'
        ): ?><div><?php endif ?><?php foreach ($this->getAddElementOption(
            $element,
            'data'
        ) as $key => $value): ?><?php if ($this->getAddElementOption(
            $element,
            'orientation'
        ) === 'horizontal'
            ...
            ): ?></div><?php endif ?><?php endforeach ?><?php if ($this->getAddElementOption(
                $element,
                'orientation'
            ) === 'horizontal'
            ): ?></div><?php endif ?>
    ...
    <?php else: ?>
        ...
        <?php /* input */ ?><?php $multicheckbox = [] ?><?php if ($this->getAddElementStorage(
            $element
        ) !== ''
        ): ?><?php $multicheckbox = explode(
            $this->getAddElementOption($element, 'separator'),
