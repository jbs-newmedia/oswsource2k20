...
if (Settings::getAction() === 'doadd') {
    ...
    $this->setDoAddElementStorage(
        ...
        osWFrame\Core\Settings::catchValue($element . $this->getAddElementOption($element, 'temp_suffix'), '', 'p')
    );

    if ((isset($_FILES[$element])) && ($_FILES[$element]['error'] === 0)) {
        if ($this->getDoAddElementStorage($element) !== '') {
            Filesystem::unlink(Settings::getStringVar('settings_abspath') . $this->getDoAddElementStorage($element));
        }
        if ($this->getDoAddElementStorage($element . $this->getAddElementOption($element, 'temp_suffix')) !== '') {
            Filesystem::unlink(
                Settings::getStringVar('settings_abspath') . $this->getDoAddElementStorage(
                    $element . $this->getAddElementOption($element, 'temp_suffix')
                )
            );
        }
        ...
        $dir = str_replace(
            '//',
            '/',
            osWFrame\Core\Settings::getStringVar('settings_abspath') . $this->getAddElementOption(
                $element,
                'file_dir'
            ) . '/'
        );
        $dir_tmp = str_replace(
            ...
            osWFrame\Core\Settings::getStringVar('settings_abspath') . $this->getAddElementOption(
                $element,
                'file_dir'
            ) . '/' . $this->getAddElementOption($element, 'file_dir_tmp') . '/'
        );
        ...
        if ($this->getAddElementOption($element, 'store_name') === true) {
            ...
            if ($this->getAddElementValue($element, 'name') !== '') {
                $this->addDataElement($element . '_name', [
                    'module' => 'hidden',
                    'name' => $this->getAddElementValue($element, 'name') . '_name',
                ]);
            }
        }
        ...
        if ($this->getAddElementOption($element, 'store_type') === true) {
            ...
            if ($this->getAddElementValue($element, 'name') !== '') {
                $this->addDataElement($element . '_type', [
                    'module' => 'hidden',
                    'name' => $this->getAddElementValue($element, 'name') . '_type',
                ]);
            }
        }
        ...
        if ($this->getAddElementOption($element, 'store_size') === true) {
            ...
            if ($this->getAddElementValue($element, 'name') !== '') {
                $this->addDataElement($element . '_size', [
                    'module' => 'hidden',
                    'name' => $this->getAddElementValue($element, 'name') . '_size',
                ]);
            }
        }
        ...
        if ($this->getAddElementOption($element, 'store_md5') === true) {
            ...
            if ($this->getAddElementValue($element, 'name') !== '') {
                $this->addDataElement($element . '_md5', [
                    'module' => 'hidden',
                    'name' => $this->getAddElementValue($element, 'name') . '_md5',
                ]);
            }
        }
        ...
        if ($this->getAddElementOption($element, 'store_sha1') === true) {
            ...
            if ($this->getAddElementValue($element, 'name') !== '') {
                $this->addDataElement($element . '_sha1', [
                    'module' => 'hidden',
                    'name' => $this->getAddElementValue($element, 'name') . '_sha1',
                ]);
            }
        }
        ...
        switch ($this->getAddElementOption($element, 'file_name')) {
            ...
            case 'shared_md5':
                ...
                $dir = str_replace(
                    '//',
                    '/',
                    Settings::getStringVar('settings_abspath') . $this->getAddElementOption(
                        $element,
                        'file_dir'
        ...
        $this->setDoAddElementStorage($element, str_replace(Settings::getStringVar('settings_abspath'), '', $file));
        $this->setDoAddElementStorage(
            $element . $this->getAddElementOption($element, 'temp_suffix'),
    ...
    if (Settings::catchValue($element . $this->getAddElementOption($element, 'delete_suffix'), '', 'p') === '1') {
        if ($this->getDoAddElementStorage($element) !== '') {
            Filesystem::unlink(Settings::getStringVar('settings_abspath') . $this->getDoAddElementStorage($element));
