...
if ($this->getAddElementOption($element, 'required') === true) {
    if ((strlen($this->getDoAddElementStorage($element)) !== 8) || ($this->getDoAddElementStorage(
        $element
    ) === '00000000')
    ) {
...
if (($this->getFilterErrorElementStorage($element) !== true) && ($this->getDoAddElementStorage(
    $element
) !== '') && ($this->getDoAddElementStorage($element) !== '00000000')
) {
    if (checkdate(
        substr($this->getDoAddElementStorage($element), 4, 2),
        substr($this->getDoAddElementStorage($element), 6, 2),
        substr($this->getDoAddElementStorage($element), 0, 4)
    ) !== true
    ) {
        $this->getTemplate()->Form()->addErrorMessage(
            $element,
