...
<div class="form-group ddm_element_<?php echo $this->getDeleteElementValue($element, 'id') ?>">
    ...
    <div class="form-control readonly">
        <?php if (($this->getDeleteElementStorage($element) === '') || ($this->getDeleteElementStorage(
            $element
        ) === '0') || ($this->getDeleteElementStorage(
            $element
        ) === '00000000') || ($this->getDeleteElementStorage($element) === '0')
        ): ?>
            ---
        <?php else: ?><?php echo $this->getTemplate()->Form()->drawHiddenField(
            $element,
            \osWFrame\Core\DateTime::strftime(
                $this->getDeleteElementOption($element, 'format'),
                mktime(
                    12,
                    0,
                    0,
                    substr($this->getDeleteElementStorage($element), 4, 2),
                    substr($this->getDeleteElementStorage($element), 6, 2),
                    substr($this->getDeleteElementStorage($element), 0, 4)
                )
            )
        ) ?><?php if ($this->getDeleteElementOption(
            $element,
            ...
            str_replace('%m.', ' %B ', $this->getDeleteElementOption($element, 'format')),
            mktime(
                12,
                0,
                0,
