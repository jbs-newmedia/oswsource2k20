...
<div class="form-group ddm_element_<?php echo $this->getDeleteElementValue($element, 'id') ?>">
    ...
    <?php /* read only */ ?>
    <?php $multicheckbox = [] ?>
    <?php if ($this->getDeleteElementStorage($element) !== ''): ?><?php $multicheckbox = explode(
        $this->getDeleteElementOption($element, 'separator'),
        $this->getDeleteElementStorage($element)
    ) ?><?php endif ?>
    <?php if ($this->getDeleteElementOption($element, 'orientation') === 'horizontal'): ?>
    <div><?php endif ?>
        <?php foreach ($this->getDeleteElementOption(
            $element,
            'data'
        ) as $key => $value): ?><?php if ($this->getDeleteElementOption(
            $element,
            'orientation'
        ) === 'horizontal'
