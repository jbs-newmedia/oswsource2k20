...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($element, 'id') ?>">
    ...
    <?php if ($this->getEditElementOption($element, 'read_only') === true): ?>
        ...
        <?php /* read only */ ?><?php $bitmask = $this->getEditElementStorage(
            $element
        ); ?><?php if ($this->getEditElementOption($element, 'orientation') === 'horizontal'): ?><div><?php endif ?>

        <?php foreach ($this->getEditElementOption(
            $element,
            'data'
            ...
            <div class="custom-checkbox">
                <?php if (isset($bitmask[$key]) && ($bitmask[$key] === '1')): ?><?php echo $this->getGroupMessage(
                    'log_char_true'
                ) . ' ' . HTML::outputString($value) ?>1
                <?php else: ?><?php echo $this->getGroupMessage('log_char_false') . ' ' . HTML::outputString($value) ?>0
                <?php endif ?>
    ...
    <?php else: ?>
        ...
        <?php /* input */ ?><?php $bitmask = $this->getEditElementStorage(
            $element
        ); ?><?php if ($this->getEditElementOption($element, 'orientation') === 'horizontal'): ?><div><?php endif ?>
