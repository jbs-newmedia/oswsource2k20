...
if ($this->getEditElementOption($element, 'required') === true) {
    if ((strlen($this->getDoEditElementStorage($element)) !== 8) || ($this->getDoEditElementStorage(
        $element
    ) === '00000000')
    ) {
...
if (($this->getFilterErrorElementStorage($element) !== true) && ($this->getDoEditElementStorage(
    $element
) !== '') && ($this->getDoEditElementStorage($element) !== '00000000')
) {
    if (checkdate(
        substr($this->getDoEditElementStorage($element), 4, 2),
        substr($this->getDoEditElementStorage($element), 6, 2),
        substr($this->getDoEditElementStorage($element), 0, 4)
    ) !== true
    ) {
        $this->getTemplate()->Form()->addErrorMessage(
            $element,
