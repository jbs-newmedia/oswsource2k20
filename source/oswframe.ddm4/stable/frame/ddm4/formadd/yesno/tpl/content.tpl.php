		...
		<?php /* input */ ?><?php foreach (str_split($this->getAddElementOption($element, 'displayorder')) as $key): ?><?php if ($key=='y'): ?>
			<div class="form-check-inline">
				<div class="form-check">
					<?php echo $this->getTemplate()->Form()->drawRadioField($element, '1', $this->getAddElementStorage($element), ['input_class'=>'form-check-input']) ?>
					<label class="form-check-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)!==null): ?> text-danger<?php endif ?>" for="<?php echo $element ?>0"><?php echo \osWFrame\Core\HTML::outputString($values['options']['text_yes']) ?></label>
				</div>
			</div>
		<?php endif ?><?php if ($key=='n'): ?>
			<div class="form-check-inline">
				<div class="form-check">
					<?php echo $this->getTemplate()->Form()->drawRadioField($element, '0', $this->getAddElementStorage($element), ['input_class'=>'form-check-input']) ?>
					<label class="form-check-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)!==null): ?> text-danger<?php endif ?>" for="<?php echo $element ?>1"><?php echo \osWFrame\Core\HTML::outputString($values['options']['text_no']) ?></label>
				</div>
			</div>
