...
<div class="form-group ddm_element_<?php echo $this->getSendElementValue($element, 'id') ?>">
	...
	<?php if ($this->getSendElementOption($element, 'read_only')===true): ?>
		...
		<?php /* read only */ ?>
		<div class="form-control readonly"><?php echo \osWFrame\Core\HTML::outputString($this->getSendElementStorage($element)); ?></div>
...
<div class="form-group ddm_element_<?php echo $this->getSendElementValue($element.'_double', 'id_double') ?>">
	...
	<?php /* label */ ?>
	<label class="form-label" for="<?php echo $element ?>"><?php echo \osWFrame\Core\HTML::outputString($this->getSendElementValue($element, 'title_double')) ?><?php if ($this->getSendElementOption($element, 'required')===true): ?><?php echo $this->getGroupMessage('form_title_required_icon') ?><?php endif ?><?php echo $this->getGroupMessage('form_title_closer') ?></label>

	<?php if ($this->getSendElementOption($element, 'read_only')===true): ?>

		<?php /* read only */ ?>
		<div class="form-control readonly"><?php echo \osWFrame\Core\HTML::outputString($this->getSendElementStorage($element)); ?></div>
		<?php echo $this->getTemplate()->Form()->drawHiddenField($element.'_double', $this->getSendElementStorage($element)) ?>

	<?php else: ?>

		<?php /* input */ ?><?php echo $this->getTemplate()->Form()->drawTextField($element.'_double', $this->getSendElementStorage($element), ['input_class'=>'form-control', 'input_errorclass'=>'is-invalid']); ?>
