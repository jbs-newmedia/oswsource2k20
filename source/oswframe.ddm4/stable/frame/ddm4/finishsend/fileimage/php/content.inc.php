...
if ($this->getDoSendElementStorage($element.$this->getSendElementOption($element, 'temp_suffix'))!='') {
	rename(\osWFrame\Core\Settings::getStringVar('settings_abspath').$this->getDoSendElementStorage($element.$this->getSendElementOption($element, 'temp_suffix')), \osWFrame\Core\Settings::getStringVar('settings_abspath').$this->getDoSendElementStorage($element));
	if (($this->getSendElementStorage($element)!='')&&($this->getSendElementStorage($element)!=$this->getDoSendElementStorage($element))) {
		if ($this->getGroupOption('enable_log')!==true) {
			\osWFrame\Core\Filesystem::unlink(\osWFrame\Core\Settings::getStringVar('settings_abspath').$this->getSendElementStorage($element));
			$this->setSendElementStorage($element, '');
		}
	}
} elseif (\osWFrame\Core\Settings::catchValue($element.$this->getSendElementOption($element, 'delete_suffix'), '', 'p')==1) {
