...
class Configure extends CoreTool
{
            ...
            if (Frame\Filesystem::isDir($directory)) {
                ...
                foreach ($files as $file) {
                    if (substr($file, -4) === '.php') {
                        if (!\in_array($file, ['__patch.php', '__create.php'], true)) {
    ...
    {
        if (isset($this->values_json[$key])) {
            return (int) ($this->values_json[$key]);
        }
    ...
    public function validateFields(): self
    {
        if (isset($this->fields)) {
            foreach ($this->fields as $config_element => $config_data) {
                ...
                if (\in_array($config_data['default_type'], ['function'], true)) {
            ...
            if (Frame\Filesystem::existsFile($filename)) {
                ...
                foreach ($values as $key => $value) {
                    if ($value['write'] === true) {
                        ...
                        switch ($value['type']) {
                            case 'integer':
                                $configure_output[] = 'osW_setVar(\'' . $key . '\', ' . (int) ($value['value']) . ');';
