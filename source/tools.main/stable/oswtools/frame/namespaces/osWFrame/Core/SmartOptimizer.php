...
class SmartOptimizer
{
    ...
    public static function getOutput(string $query_string, string $filetype): bool
    {
        ...
        $allowed_dirs = [];
        if ((Settings::getArrayVar('smartoptimizer_allowed_dirs') !== null) && (Settings::getArrayVar(
            'smartoptimizer_allowed_dirs'
        ) !== [])
        ) {
            $allowed_dirs = array_merge($allowed_dirs, Settings::getArrayVar('smartoptimizer_allowed_dirs'));
        }
        if ((Settings::getArrayVar('smartoptimizer_allowed_dirs_custom') !== null) && (Settings::getArrayVar(
        ...
        foreach ($files as $file) {
            ...
            foreach ($allowed_dirs as $a_dir) {
                if (strpos(
                    realpath(Settings::getStringVar('settings_abspath') . $file),
                    realpath(Settings::getStringVar('settings_abspath') . $a_dir)
                ) === 0
                ) {
                    $allowed_check = true;
        ...
        if ((Settings::getBoolVar('smartoptimizer_gzipcompression') === true) && (!headers_sent(
        )) && (!connection_aborted())
        ) {
        ...
        $filename = Settings::getStringVar('smartoptimizer_cacheprefix') . md5(
            $query_string . (Settings::getBoolVar('smartoptimizer_embed') ? 'embed1' : 'embed0') . (Settings::getBoolVar(
                'smartoptimizer_stripoutput'
            ) ? 'stripoutput1' : 'stripoutput0')
        ) . '.' . $filetype . (Settings::getBoolVar('smartoptimizer_gzipcompression') ? '.gz' : '');
        if ((Cache::existsCache(self::getClassName(), $filename, '') !== true) || (self::getFilesModTime(
            $files
        ) > Cache::getCacheModTime(self::getClassName(), $filename, '')) && (Settings::getBoolVar(
            'smartoptimizer_servercachecheck'
        ) === true)
        ) {
        ...
        if ((self::$ts > 0) || ((Settings::getBoolVar('smartoptimizer_clientcache') !== true) || (Settings::catchValue(
            ...
            if ($generateContent === true) {
                ...
                if (Cache::writeCache(self::getClassName(), $filename, $content, '') !== true) {
                    MessageStack::addMessage(self::getNameAsString(), 'error', [
                        ...
                        'line' => __LINE__,
                        'function' => __FUNCTION__,
                        'error' => 'Could not create cache file(' . $filename . ')',
                    ]);
                }
    ...
    public static function getOutputSingle(string $file, string $filetype): bool
    {
        ...
        $filename = Settings::getStringVar('smartoptimizer_cacheprefix') . md5(
            $file . (Settings::getBoolVar('smartoptimizer_embed') ? 'embed1' : 'embed0') . (Settings::getBoolVar(
                'smartoptimizer_stripoutput'
            ) ? 'stripoutput1' : 'stripoutput0')
        ) . '.' . $filetype . (Settings::getBoolVar('smartoptimizer_gzipcompression') ? '.gz' : '');
        if ((Cache::existsCache(self::getClassName(), $filename, '') !== true) || (self::getFilesModTime(
