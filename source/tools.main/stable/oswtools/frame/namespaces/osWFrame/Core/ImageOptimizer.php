...
class ImageOptimizer
{
    ...
    {
        if ((\in_array($key, $this->getValideOptions(), true)) && ($value !== '')) {
    ...
    public function getOutput(string $image): bool
    {
        ...
        if (($count !== 2) && ($count !== 3)) {
            MessageStack::addMessage(self::getNameAsString(), 'error', [
                ...
                'error' => 'Unsupported file structure (' . $image . ')',
            ]);
        ...
        $allowed_dirs = [];
        if ((Settings::getArrayVar('imageoptimizer_allowed_dirs') !== null) && (Settings::getArrayVar(
            'imageoptimizer_allowed_dirs'
        ) !== [])
        ) {
            $allowed_dirs = array_merge($allowed_dirs, Settings::getArrayVar('imageoptimizer_allowed_dirs'));
        }
        if ((Settings::getArrayVar('imageoptimizer_allowed_dirs_custom') !== null) && (Settings::getArrayVar(
        ...
        if ($allowed_check !== true) {
            MessageStack::addMessage(self::getNameAsString(), 'error', [
                ...
                'line' => __LINE__,
                'function' => __FUNCTION__,
                'error' => 'File out of allowed dir (' . implode(',', $allowed_dirs) . ')',
            ]);
        ...
        if (Settings::getBoolVar('imageoptimizer_protect_files') === true) {
            if (!isset($options['ps'])) {
                MessageStack::addMessage(self::getNameAsString(), 'error', [
                    ...
                    'line' => __LINE__,
                    'function' => __FUNCTION__,
                    'error' => 'Checksum not matched (' . $rel_file . ')',
                ]);
        ...
        switch ($image_type) {
            ...
            default:
                MessageStack::addMessage(self::getNameAsString(), 'error', [
                    ...
                    'line' => __LINE__,
                    'function' => __FUNCTION__,
                    'error' => 'Unsupported file type (' . $image_type . ')',
                ]);
        ...
        if ((Cache::existsCache($this->getClassName(), $filenamecache, '') !== true) || (Filesystem::getFileModTime(
            $abs_file
        ) > Cache::getCacheModTime($this->getClassName(), $filenamecache, '')) && (Settings::getBoolVar(
            'imageoptimizer_servercachecheck'
        ) === true)
        ) {
        ...
        if ((isset($options['ts'])) || (Settings::getBoolVar(
            'imageoptimizer_clientcache'
        ) !== true) || (Settings::catchValue('HTTP_IF_MODIFIED_SINCE', '', 'r') !== $mtimestr)
        ) {
            if (isset($options['ts'])) {
                $ct = 60 * 60 * 24 * 365;
            ...
            if ($generateContent === true) {
                ...
                if (Cache::writeCache(self::getClassName(), $filenamecache, $content, '') !== true) {
                    MessageStack::addMessage(self::getNameAsString(), 'error', [
                        ...
                        'line' => __LINE__,
                        'function' => __FUNCTION__,
                        'error' => 'Could not create cache file(' . $rel_file . ')',
                    ]);
                }
    ...
    protected function getImageContent(string $file, array $options): string
    {
        ...
        if (isset($options['longest'])) {
            ...
            if (\count($size) === 2) {
                $ratio = (float)((int)($size[0]) / (int)($size[1]));
            ...
            if (\count($size) === 2) {
                $osW_ImageLib->cropSquareResized((int)($size[0]), (int)($size[1]));
            } else {
