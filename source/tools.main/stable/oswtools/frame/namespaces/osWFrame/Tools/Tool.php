...
class Tool {
	...
	public function hasUpdate():bool {
		...
		if (file_exists($file)) {
			$info=json_decode(file_get_contents($file), true);
			$server_data=Server::getConnectedServer($this->getServerlist());
			if ((isset($server_data['connected']))&&($server_data['connected']===true)) {
				$package_version=Server::getUrlData($server_data['server_url'].'?action=get_version&package='.$this->getPackage().'&release='.$this->getRelease().'&version='.$info['info']['version']);
				$this->details['version_update']=$package_version;
				if (Helper::checkVersion($this->getStringValue('version'), $package_version)) {
					return true;
				}
			}
		}
