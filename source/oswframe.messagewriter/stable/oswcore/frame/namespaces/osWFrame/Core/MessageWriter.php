        ...
        if (Settings::getBoolVar('debug_write_logs') === true) {
            ...
            foreach (MessageStack::getMessages() as $key => $types) {
                if (!isset(self::$ignore[$key]) || (self::$ignore[$key] !== [])) {
                    if (Filesystem::isDir(
                        Settings::getStringVar('settings_abspath') . Settings::getStringVar('debug_path') . $key . '/'
                    ) !== true
                    ) {
                        Filesystem::makeDir(
                            Settings::getStringVar('settings_abspath') . Settings::getStringVar('debug_path') . $key . '/',
                            Settings::getIntVar('settings_chmod_dir')
                        );
                    }
                    foreach ($types as $type => $messages) {
                        if ((isset(self::$ignore[$key]) !== true) || (!\in_array($type, self::$ignore[$key], true))) {
                            $logfile = Settings::getStringVar('settings_abspath') . Settings::getStringVar(
                                'debug_path'
                            ) . $key . '/' . date('Ymd', time()) . '_' . $type . '.csv';
                            if ((Filesystem::existsFile($logfile) === true) && (filesize(
                                $logfile
                            ) > Settings::getStringVar('debug_maxsize'))
                            ) {
                                $i = 0;
                                $find = false;
                                while ($find === false) {
                                    $logfile = Settings::getStringVar('settings_abspath') . Settings::getStringVar(
                                        'debug_path'
                                    ) . $key . '/' . date('Ymd', time()) . '_' . $type . '-' . $i . '.csv';
                                    if ((Filesystem::existsFile($logfile) === true) && (filesize(
                                        $logfile
                                    ) > Settings::getStringVar('debug_maxsize'))
                                    ) {
