...
if (in_array(\osWFrame\Core\Settings::getAction(), ['about'], true)) {
    ...
    if (\osWFrame\Tools\Helper::getDoAction() === 'dochange') {
        ...
        if (!in_array($chmod_files_select, $Tool->getFileOptions(), true)) {
            $osW_Form->addErrorMessage('chmod_files_select', 'Select correct value.');
        }

        if (!in_array($chmod_directory_select, $Tool->getDirOptions(), true)) {
        ...
        if ($osW_Form->hasErrorMessages() === true) {
            ...
            $cmd = 'cd ' . \osWFrame\Core\Settings::getStringVar('settings_framepath') . $chmod_dirs[$chmod_directory] . '; find -type f -print0 | xargs -0 chmod ' . (int) $chmod_files_select . '; find -type d -print0 | xargs -0 chmod ' . (int) $chmod_directory_select . ';';
            exec($cmd);
