...
<?php $i=0;foreach(osW_Tool_ProjectManager::getInstance()->checkPackageList($packages[$sl]) as $package_name => $package_data):$i++;?>
...
<?php endforeach?>
<?php endforeach?>
<?php elseif(count(osW_Tool_ProjectManager::getInstance()->checkPackageList($packages[$sl]))>0):?>
<?php $i=0;foreach(osW_Tool_ProjectManager::getInstance()->checkPackageList($packages[$sl]) as $package_name => $package_data):$i++;?>
		<tr id="package_<?php echo md5($sl.'#'.$package_data['package'].'#'.$package_data['release'])?>">
			<td><?php echo $package_data['info']['name'];?></td>
			<td><?php echo $package_data['release'];?></td>
