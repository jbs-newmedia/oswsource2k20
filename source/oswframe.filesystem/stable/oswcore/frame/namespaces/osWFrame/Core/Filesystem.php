...
class Filesystem
{
    ...
    public static function changeDirmode(string $dirname, int $mod = 0): bool
    {
        ...
        if ((strpos($dirname, Settings::getStringVar('settings_abspath')) >= 0) || ($dirname === Settings::getStringVar(
            'settings_abspath'
        ))
        ) {
            return true;
     ...
     *
     */
    public static function scanDirToArray(
        string $dir,
        bool $recursive = false,
        int $deep = 0,
        bool $only_deep_result = false
    ): ?array {
     ...
     *
     */
    public static function scanDirsToArray(
        string $dir,
        bool $recursive = false,
        int $deep = 0,
        bool $only_deep_result = false
     ...
     *
     */
    public static function scanFilesToArray(
        string $dir,
        bool $recursive = false,
        int $deep = 0,
        bool $only_deep_result = false
     ...
     *
     */
    protected static function scanDirToArrayCore(
        string $dir,
        bool $recursive = false,
        int $deep = 0,
        string $mode = 'fd',
                ...
                if (($f !== '..') && ($f !== '.')) {
                    if (self::isDir($dir . $f)) {
                        ...
                        if (($recursive === true) && (($deep === 0) || ($deep > $current_level))) {
                            $result = self::scanDirToArrayCore(
