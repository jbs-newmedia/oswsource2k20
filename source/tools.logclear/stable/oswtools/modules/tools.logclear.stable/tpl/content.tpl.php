 ...
 * @license MIT License
 *
 * @var \osWFrame\Tools\Tool\LogClear $Tool
...
<?php else: ?>
	...
	<a href="javascript:osWTools_selectAll('#oswtools_logclear')" class="btn btn-secondary">Select all</a>
	<a href="javascript:osWTools_selectNone('#oswtools_logclear')" class="btn btn-secondary">Select none</a>
	<a href="javascript:osWTools_selectInvert('#oswtools_logclear')" class="btn btn-secondary">Invert selection</a>
	...
	<?php echo $osW_Form->startForm('oswtools_logclear_form', 'current', '', ['input_addid' => true]); ?>

	<table id="oswtools_logclear" class="table table-striped table-bordered">
		<thead>
		<tr>
		...
		<tbody>
		<?php if (count($Tool->getLogList()) > 0): ?><?php $i = 0;
		    foreach ($Tool->getLogList() as $dir):$i++; ?>
				<tr>
	...
	<?php if (count($Tool->getLogList()) > 0): ?>

		<hr/>

		<a href="javascript:$('#oswtools_logclear_form').submit()" class="btn btn-primary d-block">Clear selected directories from log</a>
