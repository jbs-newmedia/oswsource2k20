...
class Network
{
    ...
    public static function directHeader(string $link = '', int $status = 302): void
    {
    ...
    public static function getIPAddress(): string
    {
        ...
        if ((!empty($_SERVER['HTTP_X_CLUSTER_CLIENT_IP'])) && (self::validateIP(
    ...
    public static function getCurrentUrl(): string
    {
        ...
        if ($_SERVER['REQUEST_SCHEME'] === 'https') {
            if (($server_port !== Settings::getIntVar('project_ssl_port')) || (Settings::getIntVar(
                'project_ssl_port'
            ) !== Settings::getIntVar('settings_ssl_port'))
            ) {
                return $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['SERVER_NAME'] . ':' . $server_port . $_SERVER['REQUEST_URI'];
            }
        } elseif ($_SERVER['REQUEST_SCHEME'] === 'http') {
            if (($server_port !== Settings::getIntVar('project_port')) || (Settings::getIntVar(
                'project_port'
            ) !== Settings::getIntVar('settings_port'))
            ) {
                return $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['SERVER_NAME'] . ':' . $server_port . $_SERVER['REQUEST_URI'];
            }
        }
    ...
    public static function verifyIP(string $ip1, string $ip2, string $scheme = 'x.x.x.x'): bool
    {
        ...
        switch ($scheme) {
            case 'x.x.x':
                if ((\count($ip1) === 4) && (\count(
                    $ip2
                ) === 4) && ($ip1[0] === $ip2[0]) && ($ip1[1] === $ip2[1]) && ($ip1[2] === $ip2[2])
                ) {
                    return true;
                }
            ...
            default:
                if ((\count($ip1) === 4) && (\count(
                    $ip2
                ) === 4) && ($ip1[0] === $ip2[0]) && ($ip1[1] === $ip2[1]) && ($ip1[2] === $ip2[2]) && ($ip1[3] === $ip2[3])
                ) {
                    return true;
                }
     ...
     * @param array $json
     */
    public static function dieJSON(string|array $json = []): void
    {
     ...
     * @param array $js
     */
    public static function dieJS(string $js = ''): void
    {
     ...
     * @param array $css
     */
    public static function dieCSS(string $css = ''): void
    {
     ...
     * @param array $css
     */
    public static function diePNG(string $filename): void
    {
    ...
    public static function diePDF(string $filename): void
    {
