...
$Tool = new \osWFrame\Tools\Tool\ProjectClear('oswframe2k20', 'tools.projectclear', 'stable');
...
if (in_array(\osWFrame\Core\Settings::getAction(), ['about'], true)) {
...
} elseif (in_array(\osWFrame\Core\Settings::getAction(), ['settings'], true)) {
    ...
    if (\osWFrame\Tools\Helper::getDoAction() === 'dosave') {
        if ($Tool->updateSettings(explode("\n", \osWFrame\Core\Settings::catchStringPostValue('projectclear_files')), explode("\n", \osWFrame\Core\Settings::catchStringPostValue('projectclear_dirs'))) === true) {
    ...
    if (\osWFrame\Tools\Helper::getDoAction() === 'doclear') {
        $Tool->clearProject();
        \osWFrame\Core\SessionMessageStack::addMessage('session', 'success', ['msg' => 'Selected files/directories has been successfully cleared.']);
        \osWFrame\Core\Network::directHeader($osW_Template->buildhrefLink('current', 'action=start'));
    }

    $jsfiles = ['resources' . \DIRECTORY_SEPARATOR . 'js' . \DIRECTORY_SEPARATOR . 'tools.projectclear.js'];
