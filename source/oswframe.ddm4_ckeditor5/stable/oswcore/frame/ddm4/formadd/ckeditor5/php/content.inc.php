...
$file = $this->getAddElementOption($element, 'file');
if ($file === '') {
    $file = [];
}
$internallink = $this->getAddElementOption($element, 'internallink');
if ($internallink === '') {
    $internallink = [];
}
$options = $this->getAddElementOption($element, 'ckeditor5');
if ($options === '') {
    $options = [];
}

$default_conf = CKEditor5::getDefaultConf();
...
if (!isset($options['conf']['language'])) {
    $options['conf']['language'] = Language::getCurrentLanguageShort();
}

if (($internallink !== []) && (isset($internallink['loader']))) {
    $options['conf']['internallink'] = [];
    $options['conf']['internallink']['testmode'] = false;
    $options['conf']['internallink']['autocompleteurl'] = $this->getTemplate()->buildhrefLink(
        'scripts',
        'script=_ckeditor5_internallink&action=autocompleteurl&loader=' . $internallink['loader'] . '&s={searchTerm}',
        false
    );
    $options['conf']['internallink']['titleurl'] = $this->getTemplate()->buildhrefLink(
        'scripts',
        'script=_ckeditor5_internallink&action=titleurl&loader=' . $internallink['loader'] . '&s={internalLinkId}',
        false
    );
    $options['conf']['internallink']['previewurl'] = $this->getTemplate()->buildhrefLink(
        'scripts',
        'script=_ckeditor5_internallink&action=previewurl&loader=' . $internallink['loader'] . '&s={internalLinkId}',
        false
    );
}
...
$CKEditor5 = new CKEditor5('#', $element, $options['conf'], $options['then'], $options['catch']);
...
if ((!isset($options['upload'])) || (!isset($options['upload']['url']))) {
    $CKEditor5->setSimpleUploadUrl(
        $this->getTemplate()->buildhrefLink(
            'scripts',
            'script=_ckeditor5_simple_upload&session_enabled=1&var=' . md5(
                '_ckeditor5_simple_upload#' . $element . '#' . Settings::getStringVar('settings_protection_salt')
            ),
            false
        )
    );
