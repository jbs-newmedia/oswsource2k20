...
class DBStruct
{
    ...
    protected bool $protect_length = false;
    ...
    public function setProtectLength(bool $status): object
    {
        $this->protect_length = $status;

        return $this;
    }

    public function getProtectLength(): bool
    {
        return $this->protect_length;
    }
    ...
    {
        if (\in_array($type, ['varchar', 'int', 'tinyint', 'text', 'float', 'double'], true)) {
            return true;
        }
    ...
    {
        if (\in_array($options, ['primary', 'index', ''], true)) {
            return true;
        }
    ...
    {
        if (\in_array($type, ['int', 'tinyint', 'float', 'double'], true)) {
            if (\in_array($collation, ['unsigned', ''], true)) {
                return true;
            }
    ...
    {
        if (\in_array($type, ['int', 'tinyint', 'float', 'double'], true)) {
            if (\in_array($convert, ['date8', 'time4', 'minute', 'timestamp', ''], true)) {
                return true;
            }
    ...
    public function existsColumnName(string $table_name, string $column_name): bool
    {
        $QcheckData = self::getConnection();
        $QcheckData->prepare(
            'SELECT * FROM :table_api_database_column: WHERE table_name=:table_name: AND column_name=:column_name:'
        );
    ...
    public function createTable(
        ...
        if ($this->existsTableName($table_name) !== true) {
            ...
            $QinsertData->prepare(
                'INSERT INTO :table_api_database_table: (table_name, table_storage_engine, table_collation, table_comment, table_create_time, table_create_user_id, table_update_time, table_update_user_id) VALUES (:table_name:, :table_storage_engine:, :table_collation:, :table_comment:, :table_create_time:, :table_create_user_id:, :table_update_time:, :table_update_user_id:)'
            );
    ...
    public function updateTable(
        ...
        if ($this->existsTableName($table_name) === true) {
            ...
            $QupdateData->prepare(
                'UPDATE :table_api_database_table: SET table_storage_engine=:table_storage_engine:, table_collation=:table_collation:, table_comment=:table_comment:, table_update_time=:table_update_time:, table_update_user_id=:table_update_user_id: WHERE table_name=:table_name:'
            );
    ...
    public function createColumn(
        ...
        if ($this->existsTableName($table_name) === true) {
            ...
            if ($this->existsColumnName($table_name, $name) !== true) {
                ...
                if (\in_array($type, ['varchar', 'text', 'mediumtext'], true)) {
    ...
    public function updateColumn(
        ...
        if ($this->existsTableName($table_name) === true) {
            ...
            if ($this->existsColumnName($table_name, $name) === true) {
                ...
                if (\in_array($type, ['varchar', 'text'], true)) {
    ...
    public function deleteColumn(string $table_name, string $name): bool
    {
        ...
        if ($this->existsTableName($table_name) === true) {
            ...
            if ($this->existsColumnName($table_name, $name) === true) {
                $QdeleteData = self::getConnection();
                $QdeleteData->prepare(
                    'DELETE FROM :table_api_database_column: WHERE table_name=:table_name: AND column_name=:column_name:'
                );
                $QdeleteData->bindTable(':table_api_database_column:', 'api_database_column');
                $QdeleteData->bindString(':table_name:', $table_name);
    ...
    public function getLength(string $table_name, string $column_name, int $length): int
    {
        if ($this->getProtectLength() !== true) {
            return $length;
        }
        $QcheckData = self::getConnection();
        $QcheckData->prepare(
        ...
        $QcheckData->execute();
        if ($QcheckData->rowCount() === 1) {
            $data = $QcheckData->fetch();
            if ($data['column_length'] > $length) {
                return $data['column_length'];
            }
        }
    ...
    public function getStruct(): array
    {
        if ($this->data_struct === null) {
            ...
            foreach ($QcheckDataTable->query() as $table) {
                ...
                $QcheckDataColumn->prepare(
                    'SELECT * FROM :table_api_database_column: WHERE table_name=:tablename: ORDER BY column_position ASC, column_name ASC'
                );
    ...
    public function runStruct(): bool
    {
        foreach ($this->getStruct() as $table_details) {
            ...
            $QreadData->bindTable(':table:', 'pluss_' . $table_details['details']['table_name']);
            if ($QreadData->exec() === 0) {
                $key = [];
                $columns = [];
                foreach ($table_details['columns'] as $column_details) {
                    if (\in_array($column_details['column_options'], ['primary'], true)) {
                        $key[] = 'PRIMARY KEY (' . $column_details['column_name'] . ')';
                    }
                    if (\in_array($column_details['column_options'], ['index'], true)) {
                ...
                $QwriteData->prepare(
                    'CREATE TABLE :table: (:column::key:) ENGINE=:engine: COLLATE=:charset: COMMENT=:comment:;'
                );
                $QwriteData->bindTable(':table:', 'pluss_' . $table_details['details']['table_name']);
                ...
                if ($table_data['Collation'] !== $table_details['details']['table_collation']) {
                    $QwriteData = self::getConnection();
                ...
                if ($table_data['Engine'] !== $table_details['details']['table_storage_engine']) {
                    $QwriteData = self::getConnection();
                ...
                if ($table_data['Comment'] !== $table_details['details']['table_storage_engine']) {
                    $QwriteData = self::getConnection();
                ...
                foreach ($table_details['columns'] as $column_name => $column) {
                    if (!isset($columns[$column_name])) {
                        ...
                        $QwriteData->execute();
                    } elseif ($this->createColumnString($column) !== $this->createColumnStringFromMYSQL(
                        $columns[$column_name]
                    )
                    ) {
                        $QwriteData = self::getConnection();
                    ...
                    if (!isset($columns[$column_name])) {
                        if ($column['column_options'] === 'index') {
                            $QwriteData = self::getConnection();
                            $QwriteData->prepare('ALTER TABLE `:table:` ADD INDEX `:column_name:` (`:column_name:`)');
                            $QwriteData->bindTable(':table:', 'pluss_' . $table_details['details']['table_name']);
                            $QwriteData->bindRaw(':column_name:', $column_name);
                            $QwriteData->execute();
                        }
                    } else {
                        if (($column['column_options'] === 'index') && ($columns[$column_name]['Key'] !== 'MUL')) {
                            $QwriteData = self::getConnection();
                            $QwriteData->prepare('ALTER TABLE `:table:` ADD INDEX `:column_name:` (`:column_name:`)');
                            $QwriteData->bindTable(':table:', 'pluss_' . $table_details['details']['table_name']);
                            $QwriteData->bindRaw(':column_name:', $column_name);
                            $QwriteData->execute();
                        } elseif (($column['column_options'] !== 'index') && ($columns[$column_name]['Key'] === 'MUL')) {
                            $QwriteData = self::getConnection();
                            $QwriteData->prepare('ALTER TABLE `:table:` DROP INDEX `:column_name:`');
                            $QwriteData->bindTable(':table:', 'pluss_' . $table_details['details']['table_name']);
                            $QwriteData->bindRaw(':column_name:', $column_name);
                            $QwriteData->execute();
                        }
    ...
    public function createColumnString(array $column_details): string
    {
        ...
        if ($column_details['column_collation'] !== '') {
            if (\in_array($column_details['column_type'], ['int', 'tinyint', 'float', 'double'], true)) {
                $column[] = $column_details['column_collation'];
            }
            if (\in_array($column_details['column_type'], ['varchar', 'text', 'mediumtext'], true)) {
