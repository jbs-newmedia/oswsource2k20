...
class JSON
{
    ...
    public function optimizeStruct(): self
    {
        foreach ($this->table_struct as $table => $fields) {
            foreach ($fields as $field_name => $field) {
                $this->table_struct[$table][$field_name]['column_length'] = $this->optimizeLength(
                    $field['column_type'],
                    $field['column_length']
                );
            }
    ...
    public function loadTableStruct(string $table): self
    {
        ...
        $QgetData->prepare(
            'SELECT * FROM :table_api_database_column: WHERE table_name=:table_name: ORDER BY column_position ASC'
        );
        ...
        foreach ($fields as $values) {
            foreach ($values as $vkey => $vvalue) {
                ...
                if ((!isset($this->table_struct[$table][$vkey]['column_convert'])) || ($this->table_struct[$table][$vkey]['column_convert'] === '')) {
                    ...
                    $this->table_struct[$table][$vkey]['column_type'] = $this->getType(
                        $vvalue,
                        $this->table_struct[$table][$vkey]['column_length']
                    );
                }
            }
        }
                ...
                foreach ($table_elements as $row_elements) {
                    ...
                    foreach ($row_elements as $row_key => $row_value) {
                        ...
                        switch ($struct[$table]['columns'][$row_key]['column_type']) {
                            case 'int':
                            case 'tinyint':
                                $QinsertData->bindInt(':' . $row_key . ':', (int)$row_value);

                                break;
                            case 'float':
                            case 'double':
                                $QinsertData->bindFloat(
                                    ':' . $row_key . ':',
                                    str_replace(',', '.', (string)((float)$row_value))
                                );
            ...
            if (isset($struct[$table])) {
                foreach ($table_elements as $row_elements) {
                    ...
                    switch ($struct[$table]['columns'][$index_row_key]['column_type']) {
                        ...
                        case 'float':
                        case 'double':
                            $QcheckData->bindFloat(
                                ':index_row_value:',
                                str_replace(',', '.', (string)((float)$index_row_value))
                            );
                    ...
                    if ($QcheckData->rowCount() === 1) {
                        ...
                        $QupdateData->prepare(
                            'UPDATE :table: SET :fields_values: WHERE :index_row_key:=:index_row_value:'
                        );
