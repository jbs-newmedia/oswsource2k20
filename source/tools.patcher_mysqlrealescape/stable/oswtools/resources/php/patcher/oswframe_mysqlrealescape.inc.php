...
$patcher_title='MySQLRealEscape';

$patch_list=array();

$patch_list=osW_Tool_Patcher_MySQLRealEscape::getInstance()->getList(root_path.'frame');
		...
		foreach ($patch_list as $element) {
			if ((isset($_POST['element'][$element]))&&($_POST['element'][$element]==1)) {
				if (file_exists(root_path.$element)) {
					osW_Tool_Patcher_MySQLRealEscape::getInstance()->patchFile(root_path.$element);
				}
			}
		}
		$patch_list=osW_Tool_Patcher_MySQLRealEscape::getInstance()->getList(root_path.'frame');
...
<input class="btn btn-primary btn-block" type="submit" name="patch" value="Patch MySQLRealEscape"/>
<?php endif?>
