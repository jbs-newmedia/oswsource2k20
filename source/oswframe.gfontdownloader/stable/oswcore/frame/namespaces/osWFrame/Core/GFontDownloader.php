...
class GFontDownloader
{
    ...
    public function downloadFont(string $id, string $font, array $variants): bool
    {
        ...
        $css = file_get_contents(
            'https://fonts.googleapis.com/css?family=' . str_replace(' ', '+', $font) . ':' . implode(',', $variants),
            false,
            $context
        );
        $urls = [];
        ...
        if ((isset($urls[1])) && ($urls[1] !== [])) {
            $dir = Settings::getStringVar(
                'settings_abspath'
            ) . 'data' . \DIRECTORY_SEPARATOR . 'fonts' . \DIRECTORY_SEPARATOR . $id . \DIRECTORY_SEPARATOR;
            if (Filesystem::isDir($dir) !== true) {
                ...
                file_put_contents(
                    Settings::getStringVar(
                        'settings_abspath'
                    ) . 'data' . \DIRECTORY_SEPARATOR . 'fonts' . \DIRECTORY_SEPARATOR . $id . '.resource',
                    time()
                );
                Filesystem::changeFilemode(
                    Settings::getStringVar(
                        'settings_abspath'
                    ) . 'data' . \DIRECTORY_SEPARATOR . 'fonts' . \DIRECTORY_SEPARATOR . $id . '.resource'
                );
