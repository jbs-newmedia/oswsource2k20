...
class ErrorHandler
{
    ...
    public static function handleError(int $errno, string $errstr, string $errfile, int $errline): bool
    {
        switch ($errno) {
            ...
            case \E_WARNING:
            case \E_USER_WARNING:
                MessageStack::addMessage(self::getNameAsString(), 'warning', [
                    'time' => time(),
                    'errno' => $errno,
                    'errstr' => $errstr,
                    'errfile' => $errfile,
            ...
            case \E_NOTICE:
            case \E_USER_NOTICE:
                MessageStack::addMessage(self::getNameAsString(), 'notice', [
                    'time' => time(),
                    'errno' => $errno,
                    'errstr' => $errstr,
                    'errfile' => $errfile,
                ...
                break;
            default:
                MessageStack::addMessage(self::getNameAsString(), 'unknow', [
                    'time' => time(),
                    'errno' => $errno,
                    'errstr' => $errstr,
                    'errfile' => $errfile,
