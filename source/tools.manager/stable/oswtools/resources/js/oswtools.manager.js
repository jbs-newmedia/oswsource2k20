		...
		.done(function(ar_data) {
			$.each( ar_data, function(i, data) {
				if (data.version_installed=='0.0') {
					...
					oTable.$('#package_'+i).find('.manager_link').html(data.info.name);
				} else {
					if (data.options.index==true) {
						oTable.$('#package_'+i).find('.manager_link').html('<a href="../'+manager_package+'.'+manager_release+'/index.php?='+manager_session+'">'+data.info.name+'</a>');
					} else {
						oTable.$('#package_'+i).find('.manager_link').html(data.info.name);
					}
				}
