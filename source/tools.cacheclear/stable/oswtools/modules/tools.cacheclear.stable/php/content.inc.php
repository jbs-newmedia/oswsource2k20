...
if (in_array(\osWFrame\Core\Settings::getAction(), ['about'])) {
	...
	$Tool->readCacheList(\osWFrame\Core\Settings::getStringVar('settings_framepath').\osWFrame\Tools\Configure::getFrameConfig('cache_path'));
		...
		if (isset($_POST['dir'])) {
			foreach ($_POST['dir'] as $dir=>$status) {
				if (($status==1)&&(in_array($dir, $cache_dirs))) {
					\osWFrame\Core\Filesystem::delDir(\osWFrame\Core\Settings::getStringVar('settings_framepath').\osWFrame\Tools\Configure::getFrameConfig('cache_path').$dir);
					$i++;
				}
			}
		}
